...
try:
    ...
    print("ECGViewer C++ extension not found, please ensure it is built and installed.")
    IMPORT = False


@dataclass
class ViewerConfig:
    """Configuration for the Qt/C++ ECG viewer.
    ...
    Attributes:
        ...
        downsample_full: Reserved for potential downsampling of the full trace (currently unused).
        downsample_window: Reserved for potential downsampling of the active window (currently unused).
        hide_artifacts: If True, artifacts may be hidden/omitted in the view (currently unused in this wrapper).
    """
    window_s: float = 10.0
...
class ECGViewer:
    """Qt-backed ECG viewer that delegates rendering to a C++ extension.

    This class performs preprocessing (artifact detection, noise-replacement cleaning,
    fiducial detection) and converts the outputs into numpy arrays suitable for the C++
    Qt viewer.

    Args:
        t_abs: Absolute time array (seconds).
        v: ECG voltage array (V or mV, depending on input convention).
        fs: Sampling frequency (Hz).
        cfg: Optional viewer configuration; defaults to `ViewerConfig()`.
        file_prefix: Optional prefix used by the underlying viewer for exports/saves.
    """
    def __init__(self,
                 t_abs: np.ndarray,
                 v: np.ndarray,
    ...
    def _build_artifact_mask(self) -> np.ndarray:
        """
        Build a uint8 mask aligned to `self.t` with 1 at samples nearest artifact times.

        Returns:
            A uint8 array of shape (n_samples,) where artifact samples are marked with 1.
        """
        t_rel = self.t
